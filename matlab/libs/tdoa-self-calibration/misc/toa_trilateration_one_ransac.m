function [y,inliers_ut,nr_inliers,err_rms]=toa_trilateration_one_ransac(d,x,ransac_k,ransac_tol,inlierid);
...
elseif md<xdim,
    ...
    for kk = 1:ransac_k
        ...
        %%%
        ysols = real(ysols);  %% hack to make the solutions real
        for ii = 1:2;
            counter = counter+1;
            yy = ysols(:,ii);
            d_proj = toa_calc_d_from_xy(x,yy);
            ys(:,counter)=yy;
            inlieridtmp = find(abs(d_proj-d) < ransac_tol);
            nr_inliers(1,counter)=length(inlieridtmp);
            evals(1,counter)= sqrt(sum( (d_proj(inlieridtmp)-d(inlieridtmp)).^2 ));
        end
    end
    ...
    inliers0 = (abs(d_proj-d) < ransac_tol);
    inliers(inlierid)=inliers0;
    inliers_ut = find(inliers);
